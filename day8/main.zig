...
fn encode_vertex(vertex: []const u8) usize {
    var result: usize = 0;
    inline for (vertex[0..3]) |c| {
        assert(c >= 'A' and c <= 'Z');
...
const tree = struct {
    ...
    pub fn init() tree {
        var t = tree{ .nodes = undefined };
        @memset(&t.nodes, std.mem.zeroes(node));
        return t;
    }
};

fn solve_part1(t: *const tree, moves: []const u8) !usize {
    var cur = encode_vertex("AAA");
    const end = encode_vertex("ZZZ");
    var movecount: u64 = 0;
    if (!t.nodes[cur].is_set or !t.nodes[end].is_set) {
...
fn solve_part2(t: *const tree, moves: []const u8) usize {
    ...
    for (cur, 0..) |start, i| {
        ...
        while (!aux.is_end(temp)) {
            temp = switch (moves[movecount % moves.len]) {
                'L' => t.nodes[temp].left,
                'R' => t.nodes[temp].right,
                else => unreachable,
            };
            movecount += 1;
        }
...
fn parse_task(data: []const u8) task {
    ...
    while (iter.next()) |line| {
        ...
        t.nodes[code] = node{ .left = left, .right = right, .is_set = true };
        count += 1;
    }
    return task{ .tree = t, .moves = moves };
}

pub fn main() !void {
    var t = parse_task(taskData);

    std.debug.print("part 1: {any}\n", .{solve_part1(&t.tree, t.moves)});
    std.debug.print("part 2: {}\n", .{solve_part2(&t.tree, t.moves)});
}
...
test "sample 1" {
    ...
    try expect(try solve_part1(&t.tree, t.moves) == 2);
    try expect(solve_part2(&t.tree, t.moves) == 2);
}

test "sample 2" {
    var t = parse_task(sample2);
    try expect(try solve_part1(&t.tree, t.moves) == 6);
    try expect(solve_part2(&t.tree, t.moves) == 6);
}

test "sample 3" {
    var t = parse_task(sample3);
    try expect(solve_part1(&t.tree, t.moves) == error.NoSolutions);
    try expect(solve_part2(&t.tree, t.moves) == 6);
}

test "task" {
    var t = parse_task(taskData);
    try expect(try solve_part1(&t.tree, t.moves) == 16697);
    try expect(solve_part2(&t.tree, t.moves) == 10668805667831);
}
